...
本轮个人复现与完善预估 200 分钟，实际耗时由本人填报，合计 210 分钟。本轮计划与首次编码前的原始预估分开记录。
...
记录说明：实际耗时采用本人提供的 12 项分钟数，按 PSP 阶段顺序填写。该记录未由自动计时工具测得，不与首次辅助开发计时相加。

当前 Mac 上可以继续使用 Python 3、unittest、Coverage.py、Ruff、cProfile 和 SnakeViz 完成运行、测试和分析。课程页面允许 Python，测试须知没有把 Windows 指定为唯一平台；性能图条款点名 VS 2017/JProfiler，Python 工具截图能否替代仍需教师确认。

原有首次开发预估与阶段计时保留在项目 PSP 文档中。博客仍需补充个人复盘后发布。
